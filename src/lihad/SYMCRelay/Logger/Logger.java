...
public class Logger {
	...
	public void buff(int x){
		if(enabled) try {for(int i = 0;i<x;i++){writer.newLine();} writer.flush();} catch (IOException e){e.printStackTrace();}
	}
	public void noformat(String string){
		if(enabled) try { writer.write(string); writer.newLine(); writer.flush(); System.out.println(string);} catch (IOException e){e.printStackTrace();}
	}
